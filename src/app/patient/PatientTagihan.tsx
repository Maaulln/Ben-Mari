...
import { getTagihanPasien, getTagihanDetail, TagihanPasien } from '../../services/patientService';
...
export function PatientTagihan({ pasienId }: PatientTagihanProps) {
  ...
  const handleOpenDetail = async (tagihan: TagihanPasien) => {
    try {
      const detail = await getTagihanDetail(tagihan.TAGIHAN_ID);
  ...
  const tagihanBelum = tagihanList.filter((t) => t.STATUS_BAYAR === 'BELUM' || t.STATUS_BAYAR === 'CICIL');
  const tagihanLunas = tagihanList.filter((t) => t.STATUS_BAYAR === 'LUNAS');
    ...
    <div className="pb-20 md:pb-6">
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4">
          ...
          <p className="text-sm text-gray-500 mt-1">Kelola pembayaran Anda</p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {tagihanBelum.length > 0 && (
              <div>
                <h2 className="text-lg font-bold text-gray-900 mb-4">Belum Dibayar</h2>
            ...
            {tagihanLunas.length > 0 && (
              <div>
                <h2 className="text-lg font-bold text-gray-900 mb-4">Sudah Lunas</h2>
                <div className="space-y-2">
                  {tagihanLunas.map((tagihan) => (
                    ...
                    >
                      <div>
                        <p className="text-sm text-gray-600">{formatDate(tagihan.TGL_TAGIHAN)}</p>
                        <p className="font-medium text-gray-900">
                          {tagihan.appointment?.dokter?.NAMA_DOKTER}
                        </p>
                      ...
                      <Badge status="LUNAS" type="payment" />
                    </div>
                  ))}
                </div>
              </div>
        ...
        {selectedTagihan && (
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-lg p-4">
              <p className="text-sm text-gray-600 mb-1">Tanggal Tagihan</p>
              <p className="font-semibold text-gray-900">{formatDate(selectedTagihan.TGL_TAGIHAN)}</p>
            </div>
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <p className="text-gray-900">{selectedTagihan.METODE_BAYAR}</p>
